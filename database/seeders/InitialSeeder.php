...

{
    ...
    public function run()
    {
        ...
        foreach ($movies as $movie) {
            try {
                $movieToInsert = $movie;
                unset($movieToInsert['directors']);
                unset($movieToInsert['writers']);
                unset($movieToInsert['stars']);
                unset($movieToInsert['rating']);

                $insertedMovie = $movieRepository->insert($movieToInsert);

                $this->attachPersonToMovie(trim($movie['directors']), Person::ROLES['DIRECTOR'], $insertedMovie);
                $this->attachPersonToMovie(trim($movie['writers']), Person::ROLES['WRITER'], $insertedMovie);

                preg_match_all("/.*?'name'\s?:\s?'([\w\s]+)'/", $movie['stars'], $matches);
                foreach ($matches[1] as $match) {
                    $this->attachPersonToMovie(trim($match), Person::ROLES['STAR'], $insertedMovie);
                }
            } catch (Exception $e) {
    ...
    public function attachPersonToMovie(string $person_name, string $role, Movie $movie)
    {
        $personRepository = new PersonRepository();

        try {
            $person = $personRepository->find_by_name($person_name);
            $person->movies()->attach($movie, [
                'person_role' => $role
            ]);
        } catch (Exception $e) {
            try {
                $person = $personRepository->insert([
                    'name' => $person_name
                ]);
